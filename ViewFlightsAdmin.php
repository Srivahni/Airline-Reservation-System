...
<div id="services" class="container-fluid text-center">
 <?php
	...
	if (mysqli_num_rows($result)>0)
	{
       ...
       data-fixed-number='2'>");
		echo("<thead><th style=\"display: none;\"></th><th>Flight Number</th><th data-sortable='true'>Date</th><th data-sortable='true'>Departure Time</th><th data-sortable='true'>Arrival Time<th>From</th><th>To</th><th>Status</th></thead><tbody>");
	while(($row = mysqli_fetch_row($result))!=null)
	{
		echo("<tr><td id='InstanceId' style=\"display: none;\">".$row[0]."</td><td id='FlightNo'>"
		. $row[1]. "</td><td>" .$row[2]. "</td><td>" .$row[3]. "</td><td>" .$row[4]. "</td><td>".$row[5]."</td><td>".$row[6]."</td><td>$row[7]</td></tr>");
	}
		echo("</tbody></table>");
	}
